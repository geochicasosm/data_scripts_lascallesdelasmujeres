...
function writeFileMapNames() {
  ...
  fs.writeFile(path.join(__dirname, `data/mapNames.txt`), '[' + mapNamesString + ']', function (
    err
  ) {});
}
...
function isValidHighway(properties) {
  if (properties.highway) {
    return (
      ...
      properties.highway === 'bridge' ||
      properties.highway === 'secondary' ||
      properties.highway === 'path' ||
      properties.highway === 'living_street'
    );
  }
...
function isValidRoadname(roadName) {
  return (
    ...
    !(
      /\d/.test(roadName) ||
      roadName.match(/main/i) ||
      roadName.match(/Torrent/i) ||
      roadName.match(/Viaducte/i) ||
      roadName.match(/Carretera/i) ||
      roadName.match(/Túnel/i) ||
      roadName.match(/Ctra/i) ||
      roadName.match(/Viaducte/i) ||
      roadName.match(/Moll/i) ||
      roadName.match(/Accés/i) ||
      roadName.match(/Carril/i) ||
      roadName.match(/Costa/i) ||
      roadName.match(/Corriol/i) ||
      roadName.match(/Pista/i) ||
      roadName.match(/Autovia/i)
    )
  );
}
